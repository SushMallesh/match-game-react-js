...
class MatchGame extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.onPlayGame, 1000)
  }
  ...
  onPlayGameAgain = () => {
    ...
    this.setState({timeElapsedInSeconds: 60, score: 0})
  }

  onMatchImage = id => {
    const index = Math.ceil(Math.random() * (imagesList.length - 1))
    this.setState(prevState => {
      const {randomIndex, score} = prevState
      if (id === imagesList[randomIndex].id) {
        return {score: score + 1, randomIndex: index}
      }
      return {randomIndex: index, timeElapsedInSeconds: 0}
    })
  }

  renderPlayMatchGame = () => {
    const {activeTabId, randomIndex} = this.state

    const filteredImageItems = imagesList.filter(
    ...
    return (
      <ul className="images-container">
        ...
        <ul className="thumbnails-container">
          {filteredImageItems.map(eachThumbnail => (
            <Thumbnail
  ...
  render() {
    const {timeElapsedInSeconds, score} = this.state
    if (timeElapsedInSeconds === 0) {
      this.clearTimeInterval()
    }
    const isGameRunning = timeElapsedInSeconds > 0
    return (
      <>
        <NavBar userScore={score} timer={timeElapsedInSeconds} />
        <div className="match-game-container">
          {isGameRunning && this.renderPlayMatchGame()}
          {!isGameRunning && (
            <GameFinish
